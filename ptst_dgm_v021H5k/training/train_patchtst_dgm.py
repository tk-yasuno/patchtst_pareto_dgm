...
# ── Inline implementations of required classes ─────────────────────────────────
...
class FocalLoss(nn.Module):
    ...
    def forward(self, inputs: torch.Tensor, targets: torch.Tensor) -> torch.Tensor:
        """
        Args:
            inputs: logits (before sigmoid), shape [B, 1] or [B]
            targets: binary labels, shape [B, 1] or [B]
        """
...
class LoRAParams:
    ...
    def __init__(self, r: int = 8, lora_alpha: int = 16, lora_dropout: float = 0.0, bias: str = "none"):
        self.r = r
...
class PatchTSTEncoder(nn.Module):
    """Simplified PatchTST encoder with LoRA."""
    
    def __init__(self, context_len: int, patch_len: int, stride: int, d_model: int = 128, 
                 nhead: int = 8, num_layers: int = 3, lora_params: LoRAParams = None):
        super().__init__()
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        for i in range(self.num_patches):
            ...
            patches.append(x[:, start:end].unsqueeze(1))
        patches = torch.cat(patches, dim=1)  # [B, num_patches, patch_len]
...
def build_patch_tst(context_len: int, patch_len: int, stride: int, 
                   lora_params: LoRAParams = None, d_model: int = 128) -> PatchTSTEncoder:
    """Build PatchTST encoder."""
...
class PatchTSTWrapper(nn.Module):
    """Wrapper that adds classification head to PatchTST encoder."""
    
    def __init__(self, encoder: PatchTSTEncoder, dropout: float = 0.1):
        super().__init__()
        self.encoder = encoder
        self.dropout = nn.Dropout(dropout)
        # 3 output heads for 3 horizons (30d, 60d, 90d)
        self.classifier = nn.Linear(encoder.d_model, 3)
