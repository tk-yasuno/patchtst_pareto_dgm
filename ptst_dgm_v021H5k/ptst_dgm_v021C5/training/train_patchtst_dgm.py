...
PatchTST DGM training script with Joint Optimization (v0.2.1C).

Control variables (v0.2.1C):
  - Focal Loss: --focal-alpha, --focal-gamma, --w-normal (constraint: w_normal + w_anomal = 2.0)
...
Usage:
    python ptst_dgm_v021C/training/train_patchtst_dgm.py \
        --focal-alpha 0.75 --focal-gamma 1.0 \
        --w-normal 1.0 --w-anomal 1.0 \
        --patch-len 26 --stride 16 \
        --lora-rank 16 --lora-alpha 47 \
        --output-json ptst_dgm_v021C/results/eval.json
"""
...
def main() -> None:
    ...
    # v0.2.1C: Control variables with v0.2.3 best defaults
    ...
    print(f"[Train] Focal Loss: α={args.focal_alpha:.3f}  γ={args.focal_gamma:.3f}  "
          f"w_n={args.w_normal:.3f}  w_a={args.w_anomal:.3f}")
