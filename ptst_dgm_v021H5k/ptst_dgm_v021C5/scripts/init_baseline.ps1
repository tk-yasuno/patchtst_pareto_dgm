# init_baseline.ps1
# Train PatchTST baseline with v0.2.1C5 configuration, then initialize the DGM archive.
#
# v0.2.1C5 baseline uses:
#   - Fixed Architecture: patch_len=26, stride=16 (v0.2.3 best)
#   - Fixed LoRA: rank=16, alpha=47 (v0.2.3 best)
#   - Focal Loss: default alpha=0.75, gamma=1.0, w_normal=1.0, w_anomal=1.0
#
# Run once before starting the DGM loop:
#   .\ptst_dgm_v021C5\scripts\init_baseline.ps1
...
$INIT_SCRIPT  = "ptst_dgm_v021C5\scripts\init_baseline_archive.py"
$EVAL_JSON    = Join-Path $ArchiveDir "baseline_eval.json"
$OUTPUT_DIR   = Join-Path $ArchiveDir "baseline_model"

Write-Host "===== PatchTST DGM v0.2.1C5 - Baseline Initialisation =====" -ForegroundColor Cyan
...
Write-Host "Focal Loss: default (alpha=0.75, gamma=1.0, w_n=1.0, w_a=1.0)" -ForegroundColor Yellow
Write-Host ""
...
Write-Host "  .\ptst_dgm_v021C5\scripts\run_v021C5.ps1" -ForegroundColor Cyan
Write-Host ""
